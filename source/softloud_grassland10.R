...
grassland <- function(seed, dpi = 600, width = 10, height = 10, grain = 5000,
  ...
  version <- 10
  file <- make_filename("softloud_grassland", version, seed, ".jpg")
  cat("making:", file, "\n")
  
  set.seed(seed)
  bg <- rscico()
  pal <- make_palette(pin = "black", mix = bg)
  pal <- make_softloud_palette(pin = "black", mix = "black")
  bg <- pal(1)
  ...
  dat <- use_seed(seed) %>% 
    ...
    dplyr::sample_frac(.5) %>%
    dplyr::mutate(order = order + abs(x - mean(x))*100 + abs(y - mean(y))*100)
...
make_softloud_palette <- function(pin = "black", mix = "black") {
  base <- c(pin, blend("#666a67", "white"), "#23424c", "#2F3D00")
  blends <- base #purrr::map_chr(base, ~blend(.x, mix, .5))
  colorRampPalette(blends)
}
